...
export default function LoginPage() {
  ...
  /* ── cursor refs ── */
  const cursorRef = useRef<HTMLDivElement>(null); // the custom cursor element
  ...
  return (
    <>
      ...
      <div className="login-root" style={{
        ...
        {/* FORM CARD */}
        <div style={{ position:'absolute', inset:0, zIndex:10, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}>
          <div style={{ width:'100%', maxWidth:'420px', background:'rgba(255,255,255,0.06)', backdropFilter:'blur(24px)', WebkitBackdropFilter:'blur(24px)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'16px', padding:'2.5rem 2rem', boxShadow:'0 32px 64px rgba(0,0,0,0.6)' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', marginBottom:'2rem' }}>
              ...
              <h1 style={{ fontFamily:'"General Sans",sans-serif', fontWeight:600, fontSize:'1.5rem', letterSpacing:'0.35em', color:'#fff', textTransform:'uppercase', textAlign:'center' }}>SANKALP</h1>
              <p style={{ fontFamily:'"General Sans",sans-serif', fontSize:'0.72rem', letterSpacing:'0.1em', color:'rgba(255,255,255,0.45)', marginTop:'0.3rem', textTransform:'uppercase' }}>Smart Cities Mission &amp; BISAG-N</p>
            </div>

            {/* Form */}
            <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'1.1rem' }}>
              {error && <div style={{ padding:'0.75rem 1rem', background:'rgba(244,63,94,0.15)', border:'1px solid rgba(244,63,94,0.3)', borderRadius:'10px', color:'#fda4af', fontSize:'0.75rem', fontFamily:'"General Sans",sans-serif', letterSpacing:'0.04em' }}>{error}</div>}

              <div>
                <label style={{ display:'block', fontFamily:'"JetBrains Mono",monospace', fontSize:'0.65rem', letterSpacing:'0.15em', textTransform:'uppercase', color:'rgba(255,255,255,0.5)', marginBottom:'0.45rem' }}>Username</label>
                <div style={{ position:'relative' }}>
                  <span style={{ position:'absolute', left:'0.75rem', top:'50%', transform:'translateY(-50%)', color:'rgba(255,255,255,0.3)', display:'flex', alignItems:'center' }}><User size={16}/></span>
                  <input type="text" value={username} onChange={e=>setUsername(e.target.value)} placeholder="Enter username"
                    style={{ width:'100%', paddingLeft:'2.4rem', paddingRight:'1rem', paddingTop:'0.65rem', paddingBottom:'0.65rem', background:'rgba(255,255,255,0.07)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'10px', color:'#fff', fontSize:'0.875rem', fontFamily:'"General Sans",sans-serif', outline:'none', transition:'border-color 0.2s' }}
                    onFocus={e=>(e.target.style.borderColor='rgba(16,185,129,0.6)')} onBlur={e=>(e.target.style.borderColor='rgba(255,255,255,0.12)')}/>
                </div>
              </div>

              <div>
                <label style={{ display:'block', fontFamily:'"JetBrains Mono",monospace', fontSize:'0.65rem', letterSpacing:'0.15em', textTransform:'uppercase', color:'rgba(255,255,255,0.5)', marginBottom:'0.45rem' }}>Password</label>
                <div style={{ position:'relative' }}>
                  <span style={{ position:'absolute', left:'0.75rem', top:'50%', transform:'translateY(-50%)', color:'rgba(255,255,255,0.3)', display:'flex', alignItems:'center' }}><Lock size={16}/></span>
            ...
            <div style={{ marginTop:'1.75rem', paddingTop:'1.5rem', borderTop:'1px solid rgba(255,255,255,0.08)' }}>
              <p style={{ textAlign:'center', fontFamily:'"General Sans",sans-serif', fontSize:'0.7rem', color:'rgba(255,255,255,0.3)', letterSpacing:'0.06em', marginBottom:'0.75rem' }}>Demo credentials for testing</p>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.5rem' }}>
                {[['admin','admin12345'],['analyst','analyst12345'],['planner','planner12345'],['guest','guest12345']].map(([u,p])=>(
                  <div key={u} style={{ background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'8px', padding:'0.45rem 0.6rem', fontFamily:'"JetBrains Mono",monospace', fontSize:'0.65rem', color:'rgba(255,255,255,0.45)', textAlign:'center', letterSpacing:'0.04em' }}>{u} / {p}</div>
                ))}
              </div>
              <p style={{ textAlign:'center', fontFamily:'"General Sans",sans-serif', fontSize:'0.75rem', color:'rgba(255,255,255,0.35)', marginTop:'1.2rem', letterSpacing:'0.04em' }}>
                New here?{' '}
                <Link href="/register" style={{ color:'rgba(52,211,153,0.9)', textDecoration:'none', fontWeight:600 }}>Register Account</Link>
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Arrow / Hand cursor — snaps to mouse */}
        <div
          ref={cursorRef}
